...
class VariationalAutoencoder(object):
    ...
    def __init__(self, model_args):
        ...
        with tf.name_scope('input'):
            self.x = tf.placeholder(tf.float32, [None, self.args.input_size, self.args.input_size], name='x-input')

        with tf.name_scope('encoder'):
            self.h_enc1 = self.nn_layer(self.x, self.args.input_size**2, self.args.n_encoder1, 'encoder1', act=eval(self.args.activation))
        ...
        with tf.name_scope('x-output'):
            self.reconstruction = self.nn_layer(self.h_dec3, self.args.n_decoder3, self.args.input_size**2, 'x-output', act=tf.nn.sigmoid)
    ...
    def conv_layer(self, input_tensor, filter_size, in_channels, out_channels, layer_name, strides=[1, 1, 1, 1], padding='SAME', act=tf.nn.elu):
        with tf.name_scope(layer_name):
            with tf.name_scope('weights'):
                weights = self.weight_variable([filter_size, filter_size, in_channels, out_channels])
                # variable_summaries(weights, layer_name + '/weights')
            with tf.name_scope('biases'):
                biases = self.bias_variable([out_channels])
                # variable_summaries(biases, layer_name + '/biases')
            with tf.name_scope('Wx_plus_b'):
    ...
    def deconv_layer(self, input_tensor, filter_size, in_channels, out_channels, layer_name, strides=[1, 2, 2, 1], padding='SAME', act=tf.nn.elu):
        with tf.name_scope(layer_name):
            with tf.name_scope('weights'):
                weights = self.weight_variable([filter_size, filter_size, in_channels, out_channels])
                # variable_summaries(weights, layer_name + '/weights')
            ...
            output_shape = [input_size[0], input_size[1]*strides[1], input_size[2]*strides[2], input_size[3]]
            with tf.name_scope('Wx_plus_b'):
                preactivate = tf.nn.conv2d_transpose(input_tensor, weights, output_shape, strides, padding) + biases
                # tf.histogram_summary(layer_name + '/pre_activations', preactivate)
            activations = act(preactivate, 'activation')
            # tf.histogram_summary(layer_name + '/activations', activations)
            return activations
